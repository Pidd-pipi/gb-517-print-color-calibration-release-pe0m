...
import { ReworkPanel } from './common/ReworkPanel';
...
function nextPermittedStatus(config: EntityConfig, current: string, reviewer: boolean): string | null {
  const transitions: Record<string, Record<string, string | null>> = {
    ...
    // rework_pending has no generic one-click move: re-release is gated behind
    // an accepted replacement proof and is handled inside ReworkPanel.
    printRun: { setup: 'printing', printing: 'proofing', proofing: reviewer ? 'released' : 'hold', hold: 'proofing', released: reviewer ? 'hold' : null, rework_pending: null },
    colorProof: { captured: 'review', review: reviewer ? 'accepted' : null, accepted: reviewer ? 'review' : null, rejected: reviewer ? 'review' : null, invalidated: null },
