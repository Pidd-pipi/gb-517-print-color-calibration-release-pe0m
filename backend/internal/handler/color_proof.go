...
type ColorProofHandler struct{ service service.ColorProofService }

func NewColorProofHandler(s service.ColorProofService) *ColorProofHandler {
	return &ColorProofHandler{service: s}
}
...
func (h *ColorProofHandler) create(c *gin.Context) {
	var input dto.CreateColorProof
...
func (h *ColorProofHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateColorProof
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *ColorProofHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *ColorProofHandler) remove(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
