...
type PrintRun struct {
	...
	// ReworkStartedAt marks when a reviewer pulled the released batch back
	// into hold; ReworkCount accumulates every post-release rework cycle.
	ReworkStartedAt *time.Time         `json:"reworkStartedAt"`
	ReworkCount     uint               `json:"reworkCount" gorm:"not null;default:0"`
...
type PrintRunRevision struct {
	...
	// Rework state is snapshotted so the version chain shows exactly when a
	// batch was pulled back and how many rework cycles it had accumulated.
	ReworkStartedAt *time.Time `json:"reworkStartedAt"`
	ReworkCount     uint       `json:"reworkCount" gorm:"not null;default:0"`
